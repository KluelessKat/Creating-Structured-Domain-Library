...
predictor = AIUPred(force_cpu=True)  # useful on a MacBook without CUDA
...
output='/Users/katherinezhang/Downloads/Kappel_2026SpringRotation/jpl_code/Creating-Structured-Domain-Library/2_domainLibraryStructuredSeq.tsv'
...
for sequence in df["Domain Sequence"]:#Go through your domain sequences to make disordered predictions for each one
    ...
    predictions = predictor.predict_disorder(sequence)
    predictions = np.array(predictions)

    meanScoresForSequences.append(float(np.mean(predictions)))
    fracOfDisResidues.append(float(np.sum(predictions > 0.5) / len(predictions)))
