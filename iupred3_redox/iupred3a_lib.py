...
def iupred(seq, mode='long', new_smoothing=True):
    if mode == "short":
        lc = 1
        uc = 25
        wc = 10
...
def iupred_redox(seq, new_smoothing):
    return iupred(seq.replace("C", "S"), new_smoothing=new_smoothing)
...
def anchor2(seq):
    ...
    iupred_scores = iupred(seq, new_smoothing=False)
